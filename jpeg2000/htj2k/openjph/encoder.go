...
import (
	...
	"github.com/cocosip/go-dicom-codecs/jpeg2000/htj2k/openjph/colorspace"
...
func (e *Encoder) prepareIrreversibleData() [][]float32 {
	data := make([][]float32, len(e.data))
	for component := range e.data {
		data[component] = make([]float32, len(e.data[component]))
		for i, sample := range e.data[component] {
			data[component][i] = irreversibleSampleToInternal(sample, e.params.BitDepth, e.params.IsSigned)
		}
	}
	standardICT := e.params.EnableMCT && e.params.Components == 3 &&
		len(e.params.MCTBindings) == 0 && e.params.MCTMatrix == nil
	if standardICT {
		for i := range data[0] {
			data[0][i], data[1][i], data[2][i] = colorspace.ICTForwardFloat32(
...
func (e *Encoder) convertPixelData(pixelData []byte) error {
	...
	if p.InputBitsAllocated == 8 {
		...
		// Match the fo-dicom OpenJPH wrapper: each component restarts at
		// the same row offset for greater-than-8-bit input.
...
func (e *Encoder) writeCAP(buf *bytes.Buffer) error {
	...
	capability := openJPHCapability(e.quantizationInfo(), e.params.Lossless)
...
func openJPHCapability(info quantizationInfo, lossless bool) uint16 {
	magb := openJPHMAGB(info)
	bp := 0
	switch {
	case magb <= 8:
	case magb < 28:
...
func openJPHMAGB(info quantizationInfo) int {
	magb := 0
	if info.style == 0 {
		for _, exponent := range info.expn {
			magb = max(magb, exponent+info.guardBits-1)
		}
		return magb
	}

	numDecompositions := (len(info.expn) - 1) / 3
	for index, exponent := range info.expn {
		level := 0
		if index > 0 {
			level = (index - 1) / 3
		}
		nb := numDecompositions - level
		magb = max(magb, exponent+info.guardBits-nb)
	}
	return magb
}
...
func (e *Encoder) writeHTJ2KTileParts(buf *bytes.Buffer, tileIdx int, packets []t2.Packet) error {
	...
	for partIndex, data := range parts {
		...
		if err := codestream.WriteTilePart(buf, codestream.TilePart{
			TileIndex: tileIdx,
			PartIndex: partIndex,
			PartCount: partCount,
			Header:    tileHeader.Bytes(),
			Data:      data,
		}); err != nil {
			return err
		}
	...
	if e.irreversibleData != nil {
		...
		for c := range tileData {
			...
			for ty := 0; ty < height; ty++ {
				...
				dstIdx := ty * width
				copy(tileData[c][dstIdx:dstIdx+width], e.irreversibleData[c][srcIdx:srcIdx+width])
			}
		}
...
func (e *Encoder) applyIrreversibleWaveletTransform(tileData [][]float32, width, height, x0, y0 int) [][]int32 {
	transformed := make([][]int32, len(tileData))
	quantization := e.quantizationInfo()
	for c := 0; c < len(tileData); c++ {
		if e.params.NumLevels > 0 {
			wavelet.ForwardMultilevel97Float32WithParity(tileData[c], width, height, e.params.NumLevels, x0, y0)
		}
		transformed[c] = applyOpenJPHIrreversibleQuantization(
			tileData[c], width, height, x0, y0, e.params.NumLevels,
			quantization.steps, quantization.guardBits,
		)
	}
	return transformed
}
...
func (e *Encoder) collectRDPassesAndRate(blocks []*t2.PrecinctCodeBlock) ([][]t2.PassData, float64) {
	passesPerBlock := make([][]t2.PassData, 0, len(blocks))
	totalRate := 0.0
...
func (e *Encoder) computeRDLayerAllocation(passesPerBlock [][]t2.PassData, blocks []*t2.PrecinctCodeBlock, packetEncs []*t2.PacketEncoder, numLayers int, budget float64) *LayerAllocation {
	if len(e.params.LayerRates) > 0 {
...
func passBytes(passes []t2.PassData, count int) int {
	if count <= 0 {
		return 0
	}
	if count > len(passes) {
...
func (e *Encoder) collectPassesAndRate(blocks []*t2.PrecinctCodeBlock) ([][]t2.PassData, float64) {
	passesPerBlock := make([][]t2.PassData, 0, len(blocks))
	totalRate := 0.0
	for _, cb := range blocks {
		passesPerBlock = append(passesPerBlock, cb.Passes)
...
func (e *Encoder) computeBudgetAndAlloc(passesPerBlock [][]t2.PassData, numLayers int, totalRate float64, origBytes int) (float64, *LayerAllocation) {
	budget := totalRate
...
func (e *Encoder) enforceGlobalBudget(alloc *LayerAllocation, passesPerBlock [][]t2.PassData, numLayers int, totalRate, budget float64) {
...
// encodeCodeBlock encodes one OpenJPH cleanup pass.
func (e *Encoder) encodeCodeBlock(cb codeBlockInfo, _ int) *t2.PrecinctCodeBlock {
	...
	blockEnc := e.newCodeBlockEncoder(actualWidth, actualHeight, bandNumbps)
...
func (e *Encoder) newCodeBlockEncoder(width, height, bandNumbps int) *cleanup.Encoder {
	blockEnc := cleanup.NewEncoder(width, height)
	blockEnc.SetKMax(bandNumbps)
	blockEnc.SetIrreversible(!e.params.Lossless)
	return blockEnc
}
