...
func getPassBytes(passes []t2.PassData, count int) int {
	if count <= 0 {
		return 0
	}
	if count > len(passes) {
...
func computeIncrementals(passesPerBlock [][]t2.PassData) ([][]float64, [][]int, float64) {
	numBlocks := len(passesPerBlock)
...
func truncateAtLambda(passesPerBlock [][]t2.PassData, slopes [][]float64, _ [][]int, lambda float64, minPasses []int) ([]int, float64) {
	numBlocks := len(passesPerBlock)
...
func FindOptimalLambda(passesPerBlock [][]t2.PassData, targetRate float64, tolerance float64, minPasses []int) (float64, []int, float64) {
	if tolerance <= 0 {
		tolerance = 0.01
	}
...
func AllocateLayersWithLambda(passesPerBlock [][]t2.PassData, numLayers int, layerBudgets []float64, tolerance float64) *LayerAllocation {
	numBlocks := len(passesPerBlock)
	if numBlocks == 0 {
		if numLayers <= 0 {
			numLayers = 1
...
func AllocateLayersOpenJPEGThreshold(passesPerBlock [][]t2.PassData, layerBudgets []float64) *LayerAllocation {
...
func AllocateLayersOpenJPEGThresholdMeasured(passesPerBlock [][]t2.PassData, layerBudgets []float64, measurer PacketRateMeasurer) *LayerAllocation {
	numBlocks := len(passesPerBlock)
...
func rdSlopeRange(passesPerBlock [][]t2.PassData) (float64, float64) {
...
func selectOpenJPEGThreshold(passesPerBlock [][]t2.PassData, selected []int, threshold float64) []int {
...
func selectionRate(passesPerBlock [][]t2.PassData, selected []int) float64 {
	total := 0.0
	for cb, passes := range passesPerBlock {
...
func adjustSelectionToBudget(passesPerBlock [][]t2.PassData, prev []int, selected []int, targetBudget float64) []int {
	if targetBudget <= 0 {
		return selected
	}
	numBlocks := len(passesPerBlock)
