...
func (td *TileDecoder) Decode() ([][]int32, error) {
	...
	if td.cod.Transformation == 0 {
		floatComponents := make([][]float32, numComponents)
		for i, comp := range td.components {
			floatComponents[i] = comp.floatSamples
		}
		td.decodedData = td.finalizeIrreversibleSamples(floatComponents)
	}
			...
			for cby := 0; cby < numCBY; cby++ {
				for cbx := 0; cbx < numCBX; cbx++ {
					...
					cbd := &CodeBlockDecoder{
						...
						data:      info.data,
						numPasses: numPasses,
						t1Decoder: cleanup.NewDecoder(actualWidth, actualHeight),
					}
					cbd.t1Decoder.(*cleanup.Decoder).SetIrreversible(td.cod.Transformation == 0)
					if bandNumbps, ok := bandNumbpsFromQCD(td.qcd, comp.numLevels, res, band); ok {
						cbd.t1Decoder.(*cleanup.Decoder).SetCodingContext(bandNumbps, td.htj2kMissingMSBs(info, bandNumbps))
					}
...
func (td *TileDecoder) applyIDWT(comp *ComponentDecoder) error {
	...
	case 0:
		...
		if qType == 1 || qType == 2 {
			floatCoeffs = td.applyDequantizationBySubbandFloat(comp.coefficients, comp.width, comp.height, comp.numLevels, comp.x0, comp.y0)
			wavelet.InverseMultilevel97Float32WithParity(floatCoeffs, comp.width, comp.height, comp.numLevels, comp.x0, comp.y0)
			comp.floatSamples = floatCoeffs
		} else {
...
func (td *TileDecoder) finalizeIrreversibleSamples(floatComponents [][]float32) [][]int32 {
	if td.cod.MultipleComponentTransform == 1 && len(floatComponents) >= 3 {
		count := min(len(floatComponents[0]), min(len(floatComponents[1]), len(floatComponents[2])))
		r := make([]float32, count)
		g := make([]float32, count)
		b := make([]float32, count)
		for i := 0; i < count; i++ {
			r[i], g[i], b[i] = colorspace.ICTInverseFloat32(
				floatComponents[0][i], floatComponents[1][i], floatComponents[2][i],
			)
		}
		floatComponents[0], floatComponents[1], floatComponents[2] = r, g, b
	}

	result := make([][]int32, len(floatComponents))
	for i := range floatComponents {
		result[i] = wavelet.ConvertFloat32ToInt32OpenJPH(floatComponents[i], td.siz.Components[i].BitDepth())
	}
	return result
}
...
func (td *TileDecoder) applyDequantizationBySubbandFloat(coeffs []int32, width, height, numLevels, x0, y0 int) []float32 {
	if td.qcd == nil || len(td.qcd.SPqcd) == 0 {
		// No dequantization
		return wavelet.ConvertInt32ToFloat32(coeffs)
	}

	stepSizes := td.decodeQuantizationSteps(numLevels)
	if len(stepSizes) == 0 {
		return wavelet.ConvertInt32ToFloat32(coeffs)
	}
...
func (td *TileDecoder) dequantizeSubbandFloat(data []float32, x0, y0, w, h, stride int, stepSize float32) {
	if stepSize <= 0 {
		return
	}
	for y := 0; y < h; y++ {
...
func (td *TileDecoder) decodeQuantizationSteps(numLevels int) []float32 {
	if td.qcd == nil || len(td.qcd.SPqcd) == 0 {
		return nil
	}
	qType := td.qcd.QuantizationType()
	...
	steps := make([]float32, numSubbands)
	...
	case 1:
		...
		for idx := 0; idx < numSubbands; idx++ {
			...
			steps[idx] = decodeOpenJPHIrreversibleStep(expn, baseMant, openJPHBandForSubband(idx))
		}
	case 2:
		...
		for idx := 0; idx < numSubbands; idx++ {
			...
			steps[idx] = decodeOpenJPHIrreversibleStep(expn, mant, openJPHBandForSubband(idx))
		}
	}
	for idx := range steps {
		res, band := 0, 0
		...
		if kmax, ok := bandNumbpsFromQCD(td.qcd, numLevels, res, band); ok {
			steps[idx] /= float32(uint32(1) << uint(31-kmax))
		}
	}
...
func openJPHBandForSubband(idx int) int {
	if idx == 0 {
		return 0
	}
	return (idx-1)%3 + 1
...
func decodeOpenJPHIrreversibleStep(expn, mant, band int) float32 {
	bandScale := [...]float32{1, 2, 2, 4}
	delta := float32(mant|0x800) * bandScale[band]
	delta /= float32(1 << 11)
	delta /= float32(uint32(1) << expn)
	return delta
}
