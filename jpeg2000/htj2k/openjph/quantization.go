...
type openJPHIrreversibleState struct {
	kmax     int
	delta    float32
	deltaInv float32
}

func openJPHIrreversibleSubbandState(encoded uint16, guardBits, band int) openJPHIrreversibleState {
	bandScale := [...]float32{1, 2, 2, 4}
	epsilon := int(encoded >> 11)
	kmax := epsilon - 1 + guardBits
	mantissa := float32(int(encoded&0x7ff)|0x800) * bandScale[band]
	mantissa /= float32(1 << 11)
	mantissa /= float32(uint32(1) << epsilon)
	delta := mantissa / float32(uint32(1)<<(31-kmax))
	return openJPHIrreversibleState{
		kmax:     kmax,
		delta:    delta,
		deltaInv: float32(1) / delta,
	}
}

func quantizeOpenJPHIrreversible(coefficients []float32, deltaInv float32) []int32 {
	quantized := make([]int32, len(coefficients))
	for i, coefficient := range coefficients {
		quantized[i] = quantizeOpenJPHNativeSIMD(coefficient, deltaInv)
	}
	return quantized
}

func quantizeOpenJPHNativeSIMD(coefficient, deltaInv float32) int32 {
	product := coefficient * deltaInv
	return int32(math.RoundToEven(float64(product)))
}

func applyOpenJPHIrreversibleQuantization(
	coefficients []float32,
	width, height, x0, y0, numLevels int,
	encodedSteps []uint16,
	guardBits int,
) []int32 {
	quantized := make([]int32, len(coefficients))
	for res := 0; res <= numLevels; res++ {
		for _, band := range bandInfosForResolution(width, height, x0, y0, numLevels, res) {
			stepIndex := subbandIndex(numLevels, res, band.band)
			if stepIndex < 0 || stepIndex >= len(encodedSteps) {
				continue
			}
			state := openJPHIrreversibleSubbandState(encodedSteps[stepIndex], guardBits, band.band)
			for y := 0; y < band.height; y++ {
				row := (band.offsetY+y)*width + band.offsetX
				for x := 0; x < band.width; x++ {
					index := row + x
					quantized[index] = quantizeOpenJPHNativeSIMD(coefficients[index], state.deltaInv)
				}
			}
		}
	}
...
func openJPHIrreversibleCodeblockWords(coefficients []float32, deltaInv float32) []uint32 {
	quantized := quantizeOpenJPHIrreversible(coefficients, deltaInv)
	words := make([]uint32, len(quantized))
	for i, value := range quantized {
		if value < 0 {
			words[i] = 0x80000000 | uint32(-value)
		} else {
