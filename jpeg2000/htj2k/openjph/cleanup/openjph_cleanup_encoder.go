...
func (h *Encoder) encodeOpenJPHCleanup(data []int32) ([]byte, error) {
	if h.kmax <= 0 || h.kmax >= 31 {
		return nil, fmt.Errorf("invalid HTJ2K Kmax: %d", h.kmax)
	}

	cb, maxVal := openJPHCodeblockWords(data, h.kmax, h.irreversible)
...
func openJPHCodeblockWords(data []int32, kmax int, irreversible bool) ([]uint32, uint32) {
...
func (h *Encoder) encodeOJPHInitialRows(cb []uint32, p uint, mel *ojphMELWriter, vlc *ojphVLCWriter, ms *ojphMSWriter, eVal, cxVal []uint8) {
	lep := 0
	lcxp := 0
	eVal[lep] = 0
	cxVal[lcxp] = 0
...
func (h *Encoder) encodeOJPHSubsequentRows(cb []uint32, p uint, mel *ojphMELWriter, vlc *ojphVLCWriter, ms *ojphMSWriter, eVal, cxVal []uint8) {
...
func (h *Encoder) prepareOJPHInitialQuad(cb []uint32, p uint, x, offset int, rho, eQMax *int, eQ []int, s []uint32) {
...
func (h *Encoder) prepareOJPHQuad(cb []uint32, p uint, x, y, offset int, rho, eQMax *int, eQ []int, s []uint32) {
...
func (h *Encoder) prepareOJPHSample(cb []uint32, p uint, x, y, idx int, rho, eQMax *int, eQ []int, s []uint32) {
