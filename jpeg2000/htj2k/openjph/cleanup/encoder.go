...
// Encoder implements the HTJ2K High-Throughput block encoder
...
// Encoder encodes HTJ2K blocks using MagSgn, MEL, VLC and UVLC segments.
type Encoder struct {
	// Block dimensions
	width  int
	height int
...
// NewEncoder creates a new HT block encoder
...
func (h *Encoder) SetKMax(kmax int) {
	h.kmax = kmax
}

// SetIrreversible selects OpenJPH's irreversible code-block transfer, whose
// input coefficients are already quantized into the 32-bit magnitude domain.
func (h *Encoder) SetIrreversible(irreversible bool) {
	h.irreversible = irreversible
}

// Encode encodes a code-block using HTJ2K HT cleanup pass
// Reference: ITU-T T.814 | ISO/IEC 15444-15:2019
func (h *Encoder) Encode(data []int32, _ int, roishift int) ([]byte, error) {
