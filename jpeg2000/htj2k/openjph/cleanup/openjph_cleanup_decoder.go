...
func decodeOpenJPHCleanup(codeblock []byte, width, height, kmax, missingMSBs int) ([]int32, error) {
	return decodeOpenJPHCleanupWithMode(codeblock, width, height, kmax, missingMSBs, false)
}

func decodeOpenJPHCleanupWithMode(codeblock []byte, width, height, kmax, missingMSBs int, irreversible bool) ([]int32, error) {
	if len(codeblock) == 0 {
