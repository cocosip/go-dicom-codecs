...
import (
	...
	"github.com/cocosip/go-dicom-codecs/jpeg2000/openjpeg/t2"
...
func (e *Encoder) writeCOD(buf *bytes.Buffer) error {
	...
	if err := binary.Write(codData, binary.BigEndian, uint8(0)); err != nil {
		return err
	}
...
func (e *Encoder) writeTile(buf *bytes.Buffer, tileIdx, tileWidth, tileHeight, numTilesX int) error {
	...
	return codestream.WriteTilePart(buf, codestream.TilePart{
		TileIndex: tileIdx,
		PartIndex: 0,
		PartCount: 1,
...
func (e *Encoder) applyIrreversibleWaveletTransform(tileData [][]float32, width, height, x0, y0 int) [][]int32 {
	...
	transformed := make([][]int32, len(tileData))
	quantParams := e.lossyQuantizationParams()
	stepSizes := RuntimeQuantizationSteps(quantParams.EncodedSteps, e.params.NumLevels, e.params.BitDepth)
	for c := 0; c < len(tileData); c++ {
...
func (e *Encoder) encodeCodeBlock(cb codeBlockInfo, _ int) *t2.PrecinctCodeBlock {
	...
	// Create the OpenJPEG EBCOT Tier-1 block encoder.
	...
	return e.encodeSingleLayerCodeBlock(pcb, blockEnc, cbData, numPasses, roishift, bandNumbps)
}
...
func (e *Encoder) newCodeBlockEncoder(width, height, component, res, band, _ int) blockEncoder {
	blockEnc := t1.NewT1Encoder(width, height, 0)
	blockEnc.SetOrientation(band)
	blockEnc.SetNMSEDecFractionalBits(t1NMSEDecFracBits)
	if e.params.Lossless {
		level := e.params.NumLevels - res
		norm := dwtNorm53(level, band)
		weight := norm * norm / 8192.0
		blockEnc.SetDistortionWeight(weight)
	} else {
		...
		blockEnc.SetOpenJPEGDistortionParameters(mctNorm, dwtNorm97(level, band), step, log2Gain)
	}
	return blockEnc
}
...
func (e *Encoder) openJPEGRuntimeStepForBand(res, band int) float64 {
	quantParams := e.lossyQuantizationParams()
	steps := RuntimeQuantizationSteps(quantParams.EncodedSteps, e.params.NumLevels, e.params.BitDepth)
...
func (e *Encoder) encodeSingleLayerCodeBlock(pcb *t2.PrecinctCodeBlock, blockEnc blockEncoder, cbData []int32, numPasses, roishift, bandNumbps int) *t2.PrecinctCodeBlock {
